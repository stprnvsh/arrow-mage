...
#' @param direct_access Whether to return a DuckDB query object (TRUE) or a materialized data frame (FALSE)
#' @return Data frame or direct DuckDB query result
pull_data <- function(cl, identifier, zero_copy = TRUE, direct_access = FALSE) {
  # Get metadata
  metadata <- get_dataset_metadata(cl, identifier)
  ...
  # For zero-copy optimization, use direct table reference approach
  if (zero_copy) {
    # Get table reference
    table_ref <- get_table_reference(cl, identifier)
    
    # Directly access the table using DuckDB
    if (direct_access) {
      # True zero-copy: Return a lazy query object that can be further processed
      # This doesn't fetch the data yet
      access_method <- "zero-copy"
      log_access(cl, metadata$id, "read", access_method)
      
      # Create direct connection to the database containing the table
      direct_con <- duckdb::duckdb_connect(table_ref$database_path)
      
      # Return query object
      return(duckdb::duckdb_prepare(direct_con, paste0("SELECT * FROM ", table_ref$table_name)))
    } else {
      # Fetch data with direct connection but still materialize the data frame
      # This is partial zero-copy (still uses DuckDB's efficient transfer)
      access_method <- "zero-copy-materialized"
      log_access(cl, metadata$id, "read", access_method)
      
      # Create direct connection and fetch
      direct_con <- duckdb::duckdb_connect(table_ref$database_path)
      result <- duckdb::duckdb_execute(direct_con, paste0("SELECT * FROM ", table_ref$table_name))
      data <- duckdb::duckdb_fetch(result)
      
      # Close the connection
      duckdb::duckdb_disconnect(direct_con)
      
      return(data)
    }
  }
  
  # Standard access (with copy) - backward compatibility with old access_dataset approach
  table_name <- metadata$table_name
  log_access(cl, metadata$id, "read", access_method)
