...
# Assuming cpp_wrapper.jl is loaded elsewhere and defines CrossLinkCppWrapper
# Include shared memory definitions
include(joinpath(dirname(@__DIR__), "shared_memory", "shared_memory.jl"))
...
function push_data(cl::CrossLinkManager, df::Union{DataFrame, Arrow.Table}, name::Union{String, Nothing}=nothing;
    ...
    if cl.cpp_available && cl.cpp_instance !== nothing
        try
            ...
            # Prepare C Data Interface representation for Arrow table
            # This might involve serializing to bytes or directly passing C structs
            # Assuming CrossLinkCppWrapper.push expects bytes for now
            # TODO: Update this if CrossLinkCppWrapper.push changes expectation
            io = IOBuffer()
            Arrow.write(io, arrow_table)
            arrow_c_data = take!(io)
            
            # Use C++ implementation via the wrapper
            dataset_id = CrossLinkCppWrapper.push(
                cl.cpp_instance,
                arrow_c_data, # Pass the byte representation
                name === nothing ? "" : name,
...
function pull_data(cl::CrossLinkManager, identifier::String;
    ...
    if cl.cpp_available && cl.cpp_instance !== nothing
        try
            # Use C++ implementation via the wrapper - returns uintptr_t (UInt in Julia)
            stream_ptr_uint = CrossLinkCppWrapper.pull(cl.cpp_instance, identifier)
            stream_ptr = Ptr{Arrow.ArrowArrayStream}(stream_ptr_uint)

            # Check for null pointer just in case
            if stream_ptr == C_NULL
                 error("C++ pull returned a NULL pointer for identifier: $identifier")
            end
            
            cl.debug && println("Received ArrowArrayStream pointer from C++ pull: $stream_ptr")

            # Consume the stream using Arrow.Table or Arrow.RecordBatchStream
            # Arrow.Table handles the stream release automatically in its 'do' block syntax
            # or when the resulting table goes out of scope if not using 'do'.
            arrow_data = Arrow.Table(stream_ptr) # Let Arrow.jl manage the stream

            # Convert to DataFrame if requested
            if !to_arrow
                df = DataFrame(arrow_data)
                cl.debug && println("Pulled data using C++ implementation (via stream), converted to DataFrame")
                return df
            else
                cl.debug && println("Pulled data using C++ implementation (via stream) as Arrow table")
                # Note: If returning the Arrow.Table directly, the caller must ensure it
                # stays in scope long enough, or the underlying stream might be released prematurely
                # if the C++ side doesn't keep the data alive independently of the stream.
                # For safety, one might consider materializing it: `DataFrame(arrow_data) |> Arrow.Table`
                # But let's return the direct Arrow.Table for now.
                return arrow_data 
            end
        catch e
            # Log the specific error
            cl.debug && println("C++ pull (via stream) failed, falling back to Julia implementation: $e")
            # Print stack trace if debugging
            cl.debug && Base.showerror(stderr, e, Base.catch_backtrace())
            cl.debug && println()
            # Continue with regular Julia implementation fallback
        end
    end
...
function query_data(cl::CrossLinkManager, sql::String; to_arrow::Bool=false)
    ...
    if cl.cpp_available && cl.cpp_instance !== nothing
        try
             # Use C++ implementation via the wrapper - returns uintptr_t (UInt in Julia)
            stream_ptr_uint = CrossLinkCppWrapper.query(cl.cpp_instance, sql)
            stream_ptr = Ptr{Arrow.ArrowArrayStream}(stream_ptr_uint)

            if stream_ptr == C_NULL
                error("C++ query returned a NULL pointer for SQL: $sql")
            end
            
            cl.debug && println("Received ArrowArrayStream pointer from C++ query: $stream_ptr")

            # Consume the stream using Arrow.Table
            arrow_data = Arrow.Table(stream_ptr) # Let Arrow.jl manage the stream release
            
            # Convert to DataFrame if requested
            if !to_arrow
                df = DataFrame(arrow_data)
                cl.debug && println("Executed query using C++ implementation (via stream), converted to DataFrame")
                return df
            else
                cl.debug && println("Executed query using C++ implementation (via stream) as Arrow table")
                # See note in pull_data about returning Arrow.Table directly
                return arrow_data 
            end
        catch e
            # Log the specific error
            cl.debug && println("C++ query (via stream) failed, falling back to Julia implementation: $e")
             # Print stack trace if debugging
            cl.debug && Base.showerror(stderr, e, Base.catch_backtrace())
            cl.debug && println()
           # Continue with regular Julia implementation fallback
