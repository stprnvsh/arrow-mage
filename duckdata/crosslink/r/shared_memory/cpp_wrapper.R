...
#' Push a stream to CrossLink via C++ bindings
#' @export
#' @param cl CrossLink C++ connection
#' @param schema Arrow schema defining the structure of stream data
#' @param name Optional name for the stream
#' @return A list containing stream_id and writer object, or NULL if operation failed
push_stream_via_cpp <- function(cl, schema, name = "") {
  if (!inherits(cl, "crosslink_cpp_connection")) {
    if (is.list(cl) && cl$debug) message("Not a valid CrossLink C++ connection, falling back to native R implementation.")
    return(NULL)
  }
  ...
  tryCatch({
    # Convert schema to Arrow schema if needed
    if (!inherits(schema, "Schema") && requireNamespace("arrow", quietly = TRUE)) {
      # Create a dummy schema if needed - in real implementation this would be smarter
      schema <- arrow::schema(list(placeholder = arrow::int64()))
      if (cl$debug) message("Created a placeholder schema. Real implementation would use the provided schema.")
    }
    
    # Push stream using C++ bindings
    result <- crosslink_push_stream(cl$ptr, schema, name)
    
    # Return the stream ID and writer object
    list(
      stream_id = result$stream_id,
      writer = result$writer,
      
      # Add convenience methods
      write_batch = function(batch) {
        crosslink_stream_writer_write_batch(result$writer, batch)
      },
      
      close = function() {
        crosslink_stream_writer_close(result$writer)
      },
      
      get_schema = function() {
        crosslink_stream_writer_schema(result$writer)
      },
      
      get_id = function() {
        crosslink_stream_writer_id(result$writer)
      }
    )
  }, error = function(e) {
    if (cl$debug) message("Error in C++ push_stream operation: ", e$message, ", falling back to native R implementation.")
    return(NULL)
  })
}

#' Pull a stream from CrossLink via C++ bindings
#' @export
#' @param cl CrossLink C++ connection
#' @param stream_id ID of the stream to connect to
#' @return A reader object, or NULL if operation failed
pull_stream_via_cpp <- function(cl, stream_id) {
  if (!inherits(cl, "crosslink_cpp_connection")) {
    if (is.list(cl) && cl$debug) message("Not a valid CrossLink C++ connection, falling back to native R implementation.")
    return(NULL)
  }
  ...
  tryCatch({
    # Pull stream using C++ bindings
    reader_ptr <- crosslink_pull_stream(cl$ptr, stream_id)
    
    # Return the reader with convenience methods
    reader <- list(
      reader_ptr = reader_ptr,
      
      read_next_batch = function() {
        batch <- crosslink_stream_reader_read_next_batch(reader_ptr)
        
        # If batch is NULL (end of stream), return NULL
        if (is.null(batch)) {
          return(NULL)
        }
        
        # In a real implementation, this would convert to a proper arrow::RecordBatch
      ...
      get_schema = function() {
        crosslink_stream_reader_schema(reader_ptr)
      },
      
      get_id = function() {
        crosslink_stream_reader_id(reader_ptr)
      }
    )
    
    # Add iterable interface - for-loop compatibility
    class(reader) <- c("crosslink_stream_reader", class(reader))
    
    reader
  }, error = function(e) {
    if (cl$debug) message("Error in C++ pull_stream operation: ", e$message, ", falling back to native R implementation.")
    return(NULL)
  })
}

# Implement the iterator methods for crosslink_stream_reader
# This allows for: for (batch in reader) { ... }
#' @export
`[.crosslink_stream_reader` <- function(x, i) {
  if (i == 1) {
    x$read_next_batch()
  } else {
    NextMethod()
...
# Make the reader's length unknown so that for-loops continue until NULL
#' @export
length.crosslink_stream_reader <- function(x) {
  NA_integer_
}

# For readability when printing
#' @export
print.crosslink_stream_reader <- function(x, ...) {
  cat("CrossLink Stream Reader for stream ID:", x$get_id(), "\n")
  cat("Use read_next_batch() to get the next batch, or iterate with a for loop.\n")
}

# Allow as.list to collect all batches into a list
#' @export
as.list.crosslink_stream_reader <- function(x, ...) {
  batches <- list()
  i <- 1
